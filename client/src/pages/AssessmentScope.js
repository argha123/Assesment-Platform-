...
import { getAccounts, getScopes, createScope, startAssessment } from '../services/api';

const TECH_STACK_OPTIONS = [
  'AWS', 'Azure', 'GCP', 'On-Premises', 'Hybrid Cloud',
  'VMware', 'Kubernetes', 'Docker', 'Linux', 'Windows Server',
  'Oracle', 'SQL Server', 'PostgreSQL', 'MongoDB', 'Redis',
  'Cisco', 'Palo Alto', 'Fortinet', 'F5',
  'SAP', 'Salesforce', 'ServiceNow', 'Office 365', 'Google Workspace',
  'Jenkins', 'GitLab CI', 'GitHub Actions', 'Terraform', 'Ansible',
  'Splunk', 'Datadog', 'Prometheus', 'Grafana', 'ELK Stack',
  'Active Directory', 'Okta', 'CyberArk', 'Crowdstrike'
];
...
function AssessmentScope() {
  ...
  const [form, setForm] = useState({
    ...
    technology_stack: [], infrastructure_type: '',
  ...
  const loadData = async () => {
    try {
      const [accRes, scopeRes] = await Promise.all([getAccounts(), getScopes()]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      loadData();
      setForm({ account_id: '', name: '', description: '', assessment_type: 'comprehensive', technology_stack: [], infrastructure_type: '', team_size: '', departments: [], focus_areas: ['people', 'process', 'technology'] });
    } catch (error) {
    ...
    <div>
      ...
      <div style={{ marginBottom: '24px' }}>
        <button className="btn btn-primary" onClick={() => setShowForm(!showForm)}>
          {showForm ? 'Cancel' : '+ Define New Scope'}
        </button>
      </div>

      {showForm && (
        <div className="card">
          <h3 style={{ marginBottom: '20px', color: '#1a237e' }}>Define Assessment Scope</h3>
          {accounts.length === 0 ? (
              ...
              <div className="form-group">
                ...
                <div className="checkbox-grid">
                  {TECH_STACK_OPTIONS.map(tech => (
                    <div key={tech} className={`checkbox-item ${form.technology_stack.includes(tech) ? 'selected' : ''}`} onClick={() => toggleArrayItem('technology_stack', tech)}>
