...
// Audit logger middleware — attaches req.audit() helper to every request
...
// Seed question <-> control mappings (only if empty so it's safe to re-run)
const existingMapping = db.prepare('SELECT id FROM question_control_mapping LIMIT 1').get();
if (!existingMapping) {
  seedQuestionControlMappings();
}
...
function seedBenchmarks() {
  const industries = ['Technology', 'LSH-Healthcare', 'Financial Services', 'Manufacturing', 'RCTH', 'E&UPS', 'APMEA', 'F&G'];
...
function seedQuestionControlMappings() {
  // Heuristic mapping based on keyword/category overlap between questions and controls.
  // This guarantees that gap reports actually have data on first run.
  const questions = db.prepare('SELECT id, category, subcategory, question_text FROM questions').all();
  const controls = db.prepare(`
    SELECT cc.id, cc.control_name, cc.description, cc.category as control_category, cf.code as framework_code
    FROM compliance_controls cc
    JOIN compliance_frameworks cf ON cc.framework_id = cf.id
  `).all();

  if (!questions.length || !controls.length) return;

  // Topic keywords -> control matchers.
  // Each question is linked to the controls whose name/description/category contains a keyword
  // that also appears in the question category, subcategory or text.
  const topics = [
    { kw: ['access', 'identity', 'iam', 'authentication', 'authorization'], qmatch: ['access', 'identity', 'iam', 'authentication', 'authorization', 'privilege', 'mfa'] },
    { kw: ['asset', 'inventory', 'configuration', 'cmdb'],                   qmatch: ['asset', 'inventory', 'configuration', 'cmdb', 'discovery'] },
    { kw: ['risk', 'threat'],                                                qmatch: ['risk', 'threat', 'likelihood', 'impact', 'vulnerability'] },
    { kw: ['data', 'classification', 'protection', 'encryption', 'privacy', 'gdpr', 'pii'], qmatch: ['data', 'classification', 'encryption', 'privacy', 'pii', 'dlp', 'backup', 'storage'] },
    { kw: ['incident', 'response'],                                          qmatch: ['incident', 'response', 'soc', 'forensic', 'breach'] },
    { kw: ['recovery', 'business continuity', 'continuity'],                 qmatch: ['recovery', 'continuity', 'disaster', 'backup', 'rpo', 'rto'] },
    { kw: ['training', 'awareness', 'people', 'human resource'],             qmatch: ['training', 'awareness', 'culture', 'leadership', 'team', 'skill'] },
    { kw: ['monitor', 'detect', 'log', 'audit', 'siem', 'observability'],    qmatch: ['monitor', 'detect', 'log', 'audit', 'siem', 'observability', 'alert'] },
    { kw: ['network', 'firewall', 'segmentation'],                           qmatch: ['network', 'firewall', 'segmentation', 'zero trust', 'vpn'] },
    { kw: ['policy', 'governance', 'organization', 'documentation', 'compliance', 'standard'], qmatch: ['policy', 'governance', 'documentation', 'compliance', 'standard', 'framework'] },
    { kw: ['development', 'devops', 'change', 'operations'],                 qmatch: ['devops', 'cicd', 'pipeline', 'change', 'release', 'automation', 'infrastructure as code'] }
  ];

  const matchers = controls.map(c => {
    const blob = `${c.control_name || ''} ${c.description || ''} ${c.control_category || ''}`.toLowerCase();
    const matchedTopics = topics
      .filter(t => t.kw.some(k => blob.includes(k)))
      .flatMap(t => t.qmatch);
    return { control: c, matchedTopics: Array.from(new Set(matchedTopics)) };
  });
  ...
  const tx = db.transaction(() => {
    let total = 0;
    questions.forEach(q => {
      const qBlob = `${q.category || ''} ${q.subcategory || ''} ${q.question_text || ''}`.toLowerCase();
      matchers.forEach(({ control, matchedTopics }) => {
        if (!matchedTopics.length) return;
        const hit = matchedTopics.some(t => qBlob.includes(t));
        if (hit) {
          insert.run(uuidv4(), q.id, control.id);
          total++;
        }
      });
    });
    console.log(`Question→Control mappings seeded: ${total}`);
  });
  tx();
}
