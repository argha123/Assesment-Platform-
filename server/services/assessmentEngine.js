...
function generateAssessmentQuestions(scope) {
  ...
  // Filter by technology stack if specified — include questions that either:
  // 1. Have no applicable_to filter (general questions), OR
  // 2. Have at least one applicable_to entry matching the selected tech stack
  if (techStack.length > 0) {
    questions = questions.filter(q => {
      ...
      // If no applicable_to defined, always include (general question)
      if (applicable.length === 0) return true;
      // Check if any applicable_to entry matches any selected tech
  ...
  if (scope.framework_id) {
    const mappedRows = db.prepare(`
      SELECT DISTINCT qcm.question_id
      FROM question_control_mapping qcm
      JOIN compliance_controls cc ON qcm.control_id = cc.id
      WHERE cc.framework_id = ?
    `).all(scope.framework_id);
    const allowed = new Set(mappedRows.map(r => r.question_id));
    if (allowed.size > 0) {
