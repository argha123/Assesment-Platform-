...
router.post('/scopes', (req, res) => {
  try {
    const db = getDb();
    const id = uuidv4();
    const { account_id, name, description, assessment_type, technology_stack, infrastructure_type, team_size, departments, focus_areas, framework_id } = req.body;
    
    db.prepare(`
      INSERT INTO assessment_scopes (id, account_id, name, description, assessment_type, technology_stack, infrastructure_type, team_size, departments, focus_areas, framework_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, account_id, name, description, assessment_type, 
      JSON.stringify(technology_stack), infrastructure_type, team_size, 
      JSON.stringify(departments), JSON.stringify(focus_areas), framework_id || null);
...
router.post('/start', (req, res) => {
  try {
    ...
    req.audit({ action: 'create', entity_type: 'assessment', entity_id: id, details: `Assessment started: "${title}"` });
...
router.get('/:id', (req, res) => {
  try {
    ...
    const assessment = db.prepare(`
      SELECT a.*, s.name as scope_name, s.technology_stack, s.focus_areas, s.departments, s.framework_id,
        cf.name as framework_name, cf.code as framework_code,
        acc.name as account_name
      FROM assessments a
      JOIN assessment_scopes s ON a.scope_id = s.id
      LEFT JOIN compliance_frameworks cf ON s.framework_id = cf.id
...
router.get('/', (req, res) => {
  try {
    ...
    const assessments = db.prepare(`
      SELECT a.*, s.name as scope_name, s.framework_id,
...
router.post('/:id/complete', (req, res) => {
  try {
    ...
    req.audit({ action: 'complete', entity_type: 'assessment', entity_id: assessmentId, details: `Assessment completed. Score: ${scores.overall}/10` });
